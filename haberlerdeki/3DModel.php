...
<body>
    <?php
        ...
        $strSuccessURL = "http://localhost/odeme_sis/3DModelResults.php";
        $strErrorURL = "http://localhost/odeme_sis/3DModelResults.php";
        ...
        $HashData = strtoupper(sha1($strTerminalID.$strOrderID.$strAmount.$strSuccessURL.$strErrorURL.$strType.$strInstallmentCount.$strStoreKey.$SecurityData));
		    ...
		    //   $SecurityData             = strtoupper(sha1($strProvisionPassword.$strTerminalID_ ));
            //    $HashData                 = strtoupper(sha1($strOrderID.$strTerminalID.$strAmount.$SecurityData)); //Daha kisitli bilgileri HASH ediyoruz
    ...
    <form action="https://sanalposprovtest.garanti.com.tr/servlet/gt3dengine " method="post">
        ...
        <br />
        Card Number: <input name="cardnumber" value="375622005485014" type="text" />
        <br />
        Expire Date (mm): <input name="cardexpiredatemonth" value="10" type="text" />
        <br />
        Expire Date (yy): <input name="cardexpiredateyear" value="20" type="text" />
        <br />
